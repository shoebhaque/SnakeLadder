...
public class Player {
    ...
    private static  Board gameBoard = new Board();    //it is doesnt matter how many player playing game the board is one thats why static
    public Player(int tileSize, Color coinColor,String playerName){
        coin = new Circle(tileSize/2); //passing the radius of circle
        coin.setFill(coinColor);  // set color
        currentPosition = 0;   // we can set whatever we want the position 0 etc
        movePlayer(1);
        name = playerName;
    }
...
//        coin.setTranslateY(y);
    ...
    private TranslateTransition  translateAnimation(int diceValue){ //write a logoc for proper playing game
      //it will move slowly
